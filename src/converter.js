...
function convertArticle(html, baseUrl) {
  ...
  if (!article) {
    // Readability failed — likely SPA or unusual page structure
...
async function convert(url, mode = 'article') {
  ...
  // Detect SPA shell (pages that are mostly JS with no real content)
  ...
  // Try article mode; if it fails with readability error, fall back to full page
  if (mode === 'article') {
    try {
